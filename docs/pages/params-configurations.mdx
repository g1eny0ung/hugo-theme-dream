...
To enable Twikoo, you need to set the `enabled` parameter to `true` and set the following 4 parameters referring to the [official manual](https://twikoo.js.org/quick-start.html)
...
<Callout type="info">
You still need to set the `enabled` parameter to `true` to enable Twikoo when using a custom partial file.

Other related parameters and **CDN import** will be ingored.
</Callout>
