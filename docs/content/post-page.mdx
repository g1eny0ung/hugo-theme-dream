...
Pan and zoom are disabled by default. Enable them for an individual diagram by adding `zoom="true"` to its code block attributes:
...
Drag the enabled diagram to pan it, use the mouse wheel to zoom, and hover over it to reveal a reset button in the bottom-right corner. Clicking the button restores the initial zoom and position.
